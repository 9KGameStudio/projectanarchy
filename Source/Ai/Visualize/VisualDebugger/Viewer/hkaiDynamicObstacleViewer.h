...
#ifndef HKAI_DYNAMIC_OBSTACLE_VIEWER_H
#define HKAI_DYNAMIC_OBSTACLE_VIEWER_H
...
/// Viewer to display dynamic obstacles in the AI world.
class HK_EXPORT_AI hkaiDynamicObstacleViewer : public hkaiViewerBase,
...
public:
	...
	/// Registers hkaiDynamicObstacleViewer with the hkViewerFactory.
	static void HK_CALL registerViewer();

	/// Returns the viewer name. This text appears in the VDB client viewer menu.
	static inline const char* HK_CALL getName() { return "AI - Dynamic Obstacles"; }
...
protected:
	...
	hkaiDynamicObstacleViewer(const hkArray<hkProcessContext*>& contexts);
	virtual ~hkaiDynamicObstacleViewer();
