...
extern HK_EXPORT_AI const class hkClass hkaiNavMeshQueryMediatorClass;

	/// This is an interface for proximity and ray cast queries against nav mesh faces
class HK_EXPORT_AI hkaiNavMeshQueryMediator : public hkReferencedObject
{
public:
	...
	struct HK_EXPORT_AI QueryInputBase
	{
		...
		hkUint32 m_filterInfo;
			...
			/// Layer bitfield. Queries will only "hit" mesh instances whose layers overlap with the query's layers.
		hkaiLayer m_layers;
	...
	struct HK_EXPORT_AI GetClosestPointInput : public QueryInputBase
	{
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_AI, GetClosestPointInput);
	...
	struct HK_EXPORT_AI RaycastInput : public QueryInputBase
	{
		...
		/// Input information for sphere cast queries
	struct HK_EXPORT_AI SphereCastInput : public RaycastInput
	{
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_AI, SphereCastInput);

		///
		inline SphereCastInput();
	...
	struct HK_EXPORT_AI BidirectionalRaycastInput : public QueryInputBase
	{
	...
	struct HK_EXPORT_AI HitDetails 
	{
	...
	virtual bool castRay( const RaycastInput& input, HitDetails& hitOut ) const = 0;
		...
		/// Returns the HitDetails for the closest face that the sphere hits.
	virtual bool castSphere( const SphereCastInput& input, HitDetails& hitOut ) const = 0;
	...
	struct HK_EXPORT_AI AabbQueryInput : public QueryInputBase
	{
		...
		///
		inline AabbQueryInput( const QueryInputBase& base );
	...
	struct HK_EXPORT_AI CoherentInput
	{
	...
	bool castSphere( hkVector4Parameter from, hkVector4Parameter to, hkSimdRealParameter radius, HitDetails& hitOut ) const
	{
		SphereCastInput input;
		input.m_from = from;
		input.m_to = to;
		input.m_radius = radius;
		return castSphere(input, hitOut);
	}
	...
	static inline hkBool32 HK_CALL isInstanceEnabled( const QueryInputBase& input, const hkaiNavMeshInstance* instance )
	{
		if(instance)
		{
			const hkaiLayer instanceLayer = instance->getLayer();
			if ( (input.m_layers & instanceLayer) == 0 )
			{
				return false;
			}
		}
		...
		/// Utility method to forward the QueryInputBase fields to its hit filter (if non-NULL)
	static inline hkBool32 HK_CALL isFaceEnabled( const QueryInputBase& input, const hkaiNavMeshInstance* instance, hkaiNavMeshFaceIndex faceIndex )
	{
