...
extern HK_EXPORT_AI const class hkClass hkaiNavVolumeMediatorClass;
class hkaiNavVolume;

	/// Interface for spatial queries on an hkaiNavVolume.
class HK_EXPORT_AI hkaiNavVolumeMediator : public hkReferencedObject
{
public:
	...
	{
		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_AI, QueryInputBase);

			/// Default constructor - m_position is set to HK_REAL_MAX, and m_queryRadius is set to 5.0
		inline QueryInputBase();
	...
	struct HK_EXPORT_AI AabbQueryInput : public QueryInputBase
	{
		...
		/// Returns the array of cell keys that are touched by the AABB.
	...
	static inline hkBool32 HK_CALL isVolumeEnabled( const QueryInputBase& input, const hkaiNavVolumeInstance* volumeInstance)
	{
		if(volumeInstance)
		{
			const hkaiLayer instanceLayer = volumeInstance->getLayer();
			if ( (input.m_layers & instanceLayer) == 0 )
			{
				return false;
			}
		}
		return !input.m_hitFilter || input.m_hitFilter->isVolumeEnabled( volumeInstance, input.m_filterInfo, input.m_userData) ;
	}

		/// Utility method to forward the GetClosestPointInput fields to its hit filter (if non-NULL)
	static inline hkBool32 HK_CALL isCellEnabled( const QueryInputBase& input, const hkaiNavVolumeInstance* volumeInstance, hkaiNavVolume::CellIndex cellIndex)
	{
		return !input.m_hitFilter || input.m_hitFilter->isCellEnabled( volumeInstance, cellIndex, input.m_filterInfo, input.m_userData) ;
	}
