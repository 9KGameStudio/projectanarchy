...
namespace hkaiTraversalAnalysisUtils
{
	...
	void findOverLedgeReachability(
		...
		hkSimdRealParameter verticalLip,
		hkaiTraversalAnalysisContext const& context,
	...
	/// Expand the segment in both directions, such that each point is further 
	/// out in the horizontal by the character radius. For non-horizontal
	/// segments, no point on the new segment is lower than the lowest point on
	/// the original segment.
	/// 
	/// @param contractionAmountOut Filled with the offset of the original
	/// endpoints in the interpolation space of the new edge. This can be passed
	/// to contractIntervalPartitionByWidth.
	void expandSegmentByRadius(
		hkVector4 & leftInOut, hkVector4 & rightInOut, 
		hkReal &contractionAmountOut,
		hkSimdRealParameter characterRadius, hkVector4Parameter up);

	/// Contract an interval partition from a cast of a segment which was 
	/// generated by expandSegmentByWidth. This has the effect of contracting the
	/// overall cast by the character width, while remapping the partition to
	/// the parameters space of the endpoints originally passed to 
	/// expandSegmentByWidth.
	void contractIntervalPartitionByRadius(
		hkaiIntervalPartition & partitionInOut, 
		hkReal contractionAmount);
}
