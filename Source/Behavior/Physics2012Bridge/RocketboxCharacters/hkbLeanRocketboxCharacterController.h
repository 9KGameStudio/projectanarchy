...
#ifndef HKB_LEAN_ROCKETBOX_CHARACTER_CONTROLLER_H
#define HKB_LEAN_ROCKETBOX_CHARACTER_CONTROLLER_H

#include <Behavior/Behavior/hkbBehavior.h>
#include <Behavior/Behavior/Generator/Modifier/hkbModifierGenerator.h>

extern const class hkClass hkbLeanRocketboxCharacterControllerClass;


class hkbLeanRocketboxCharacterController : public hkbModifier
{
public:
	//+vtable(true)
	//+version(0)
	...
	hkbLeanRocketboxCharacterController();
	~hkbLeanRocketboxCharacterController();
	hkbLeanRocketboxCharacterController(hkFinishLoadedObjectFlag flag);

protected:
	hkbLeanRocketboxCharacterController(hkbLeanRocketboxCharacterController const& other);
...
public:
	...
	// hkbNode interface implementation.
	virtual hkbLeanRocketboxCharacterController* cloneNode( hkbBehaviorGraph& rootBehavior ) const HK_OVERRIDE;
...
public:
	...
	hkReal					m_rotationSpeed;			//hk.description("How fast can a character rotate");

	int						m_poseIdx;					//hk.description("Desired pose index.")

	int						m_rotationAllowed;			//hk.description("Can the character rotate.")
