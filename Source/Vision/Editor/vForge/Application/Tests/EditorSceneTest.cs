...
namespace Editor.Tests
{
  ...
  public class EditorSceneTest
    ...
    /// <summary>
    /// Make project loading fail by emulating a video initialization problem. Subsequent project loads should not be affected.
    /// </summary>
    [Test]
    public void LoadFailTest()
    {
      try
      {
        string  project = Path.Combine(TestManager.Helpers.TestDataDir, @"ComponentTest\ComponentTest.project"); // arbitrary test project
        Assert.IsTrue(File.Exists(project));

        EditorManager.EngineManager.ForceVideoInitFail = true;

        Assert.IsFalse(EditorApp.EditorAppDelegates.LoadProjectDelegate(project, false));
      }
      finally
      {
        EditorManager.EngineManager.ForceVideoInitFail = false;
      }
    }
