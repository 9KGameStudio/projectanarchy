...
{
  /// \brief Managed property class for Hinge constraints.
  /// 
  /// The HingeConstraintState class exposes the properties for hinge
  /// constraints and provides functions for setting/retrieving them on the 
  /// native description.
  [Serializable]
  public ref class HingeConstraintState : public ConstraintStateBase
  {
  public:
    /// Constructor. Sets the default values.
    HingeConstraintState();

    [PropertyOrder(102)]
    [Description("Properties for the maximum and minimum angular limits settings.")]
    [TypeConverter(UndoableObjectConverter::typeid)]
    property ConstraintLimitsDesc^ LimitsSettings
    {
      ConstraintLimitsDesc^ get() { return m_pLimitsDesc; }
      void set(ConstraintLimitsDesc ^pLimitsDesc) { m_pLimitsDesc = pLimitsDesc; }
    }
  ...
  protected:
    ...
    HingeConstraintState(SerializationInfo ^si, StreamingContext sc);
    ...
    // Variables
    ConstraintLimitsDesc ^m_pLimitsDesc;
  };
}
