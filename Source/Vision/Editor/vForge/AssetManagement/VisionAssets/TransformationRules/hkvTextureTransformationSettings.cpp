...
void hkvTextureTransformationSettings::setSourceProperties(bool srgb, bool hasAlpha, hkUint32 width, hkUint32 height)
{
  m_sourceSrgb = srgb;
...
void hkvTextureTransformationSettings::setUsage(hkvTextureUsage usage)
{
  m_usage = usage;
}
  ...
  hkArray<hkvAssetLogMessage>& out_messages)
{
  ...
  case HKV_TARGET_PLATFORM_TIZEN:
    {
...
bool hkvTextureTransformationSettings::checkDimensions(hkArray<hkvAssetLogMessage>& out_messages)
{
  ...
  if ((m_targetWidth != m_sourceWidth) || (m_targetHeight != m_sourceHeight))
  {
    ...
    out_messages.pushBack(hkvAssetLogMessage(HKV_MESSAGE_CATEGORY_ASSET_TRANSFORMATION, HKV_MESSAGE_SEVERITY_INFO, msg));
  }
  ...
  if ((m_validationWarnSize > 0) && ((m_targetWidth > m_validationWarnSize) || (m_targetHeight > m_validationWarnSize)))
  {
    ...
    out_messages.pushBack(hkvAssetLogMessage(HKV_MESSAGE_CATEGORY_ASSET_TRANSFORMATION, HKV_MESSAGE_SEVERITY_WARNING, msg));
  }
  ...
  hkArray<hkvAssetLogMessage>& out_messages)
{
  ...
  {
    out_messages.pushBack(hkvAssetLogMessage(HKV_MESSAGE_CATEGORY_ASSET_TRANSFORMATION, HKV_MESSAGE_SEVERITY_WARNING, "Alpha channel is discarded due to target data format restrictions!"));
  }
