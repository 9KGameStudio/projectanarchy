...
bool hkvCollectionTransformationRule::executeTransformation(const hkvTransformationInput& input, hkvTransformationOutput& output) const
{
  ...
  if (ok)
  {
    ...
    context.m_output.m_messages.pushBack(hkvAssetLogMessage(
      HKV_MESSAGE_CATEGORY_ASSET_TRANSFORMATION, HKV_MESSAGE_SEVERITY_INFO, "Collection transformation succeeded."));
  }
  else
  {
    context.m_output.m_messages.pushBack(hkvAssetLogMessage(
      HKV_MESSAGE_CATEGORY_ASSET_TRANSFORMATION, HKV_MESSAGE_SEVERITY_ERROR, "Collection transformation failed!"));
  }
