...
void RPG_BossAiControllerComponent::SetOwner(VisTypedEngineObject_cl *newOwner)
{
  ...
  if(newOwner)
  {
    ...
    m_states[RPG_ControllerStateId::kIdling] = new RPG_BossAiControllerState::Idling;
    m_states[RPG_ControllerStateId::kMoving] = new RPG_BossAiControllerState::Moving;
...
void RPG_BossAiControllerState::Idling::OnTick(RPG_ControllerComponent *const controller, float const deltaTime)
{
...
void RPG_BossAiControllerState::Moving::OnTick(RPG_ControllerComponent *const controller, float const deltaTime)
{
  ...
  if(aiController->HasValidTarget())
  {
    ...
    {
      controller->RequestPath(target->GetPosition());
    }
    else
    {
