...
public:
  ...
  enum State
  {
    CANCELLED = -3, ///< The dialog was cancelled by some external means, e.g. by pressing the back button on the device.
    WAITING = -2,   ///< There was no user input yet.
  ...
  /// \brief Destructor.
  virtual ~IVNativeDialog() {}

  /// \brief Creates an instance of the dialog.
  static IVNativeDialog* CreateInstance();

  /// \brief Sets the title of the dialog.
  virtual void SetTitle(const char* pTitle) = 0;

  /// \brief Sets the text message of the dialog. Changing the text after calling Show() works on all platforms.
