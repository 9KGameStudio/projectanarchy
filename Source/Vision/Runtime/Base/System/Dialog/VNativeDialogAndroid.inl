...
public:
  ...
  virtual void SetText(const char* pText) HKV_OVERRIDE
  {
    ...
    if(m_dialog)
    {
      ...
      jmethodID method_VNativeMobileDialogAndroid_ChangeText = env->GetStaticMethodID(m_clazz_Dialog, "ChangeText",
        "(Landroid/app/Activity;Lcom/havok/Vision/VNativeDialogAndroid;Ljava/lang/String;)V");
  ...
  {
    // Allow running the app while the dialog is shown in the foreground
...
IVNativeDialog* IVNativeDialog::CreateInstance()
{
  return new VNativeDialogAndroid();
}

extern "C" JNIEXPORT void JNICALL Java_com_havok_Vision_VNativeDialogAndroid_SetDialogState(JNIEnv* env, jclass clazz, jlong nativePointer, jint which)
{
  reinterpret_cast<VNativeDialogAndroid*>(nativePointer)->m_eState = static_cast<IVNativeDialog::State>(which);
}

extern "C" JNIEXPORT void JNICALL Java_com_havok_Vision_VNativeDialogAndroid_SetDialogInstance(JNIEnv* env, jclass clazz, jlong nativePointer, jobject dialog)
{
  reinterpret_cast<VNativeDialogAndroid*>(nativePointer)->m_dialog = env->NewGlobalRef(dialog);
}
