...
bool VGraphObject::AddCurve(const char *szName, VColorRef iColor, VCurveUpdater* pUpdater)
{
  if (m_bInitialized)
  {
    hkvLog::Warning("VGraphObject: Curve must be added before object is initialized!");
    return false;
  }
  ...
  // set updater which is responsible for delivering new values every frame
  VASSERT_MSG(pUpdater, "Tried to add cruve without a valid updater!");
  curve.m_spUpdater = pUpdater;

  // create screen-mask for legend of curve  
  curve.m_spLegendMask = new VisScreenMask_cl("plainwhite.dds");
  curve.m_spLegendMask->SetColor(iColor);
  curve.m_spLegendMask->SetDepthWrite(FALSE);
  curve.m_spLegendMask->SetVisibleBitmask(VIS_ENTITY_VISIBLE_IN_WORLD);
  curve.m_spLegendMask->SetTargetSize(VGRAPH_LEGEND_SIZE, VGRAPH_LEGEND_SIZE);
...
void VGraphObject::Init(const char *szMaskTexture)
{
  ...
  m_spBackgroundMask->SetVisibleBitmask(VIS_ENTITY_VISIBLE_IN_WORLD);
...
void VGraphObject::SetVisible(bool bVisible)
{
  if (!m_bInitialized)
  {
    hkvLog::Warning("VGraphObject: Object is not initialized!");
    return;
  }
  ...
  if (m_spBackgroundMask)
    m_spBackgroundMask->SetVisibleBitmask(bVis ? VIS_ENTITY_VISIBLE_IN_WORLD : VIS_ENTITY_INVISIBLE);

  for (int i=0; i<m_curves.GetSize(); ++i)
  {
    if (m_curves[i].m_spLegendMask)
      m_curves[i].m_spLegendMask->SetVisibleBitmask(bVis ? VIS_ENTITY_VISIBLE_IN_WORLD : VIS_ENTITY_INVISIBLE);
  }
...
void VGraphObject::OnHandleCallback(IVisCallbackDataObject_cl *pData)
{
  if (pData->m_pSender == &Vision::Callbacks.OnFrameUpdatePreRender)
  {
    ...
    for (int i=0; i<m_curves.GetSize(); ++i)
    {
      m_curves[i].m_spUpdater->Update(values);
      for (int j=0; j<values.GetSize(); ++j)
        AppendValue(i, values[j]);

      values.SetSize(0, 0, false);
    }
...
void VGraphObject::AppendValue(int iCurveIndex, float fValue)
{
  VASSERT(iCurveIndex < m_curves.GetSize());
