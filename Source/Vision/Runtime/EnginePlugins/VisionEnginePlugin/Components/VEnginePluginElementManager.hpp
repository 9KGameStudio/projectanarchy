...
#ifndef V_ENGINE_PLUGIN_ELEMENT_MANAGER_HPP_INCLUDED
#define V_ENGINE_PLUGIN_ELEMENT_MANAGER_HPP_INCLUDED


#define V_ENGINE_PLUGIN_ELEMENT_MANAGER_FREE_INDEX_CACHE_SIZE  128

/// \brief
///   Template element manager class for classes derived from VisObjectKey_cl inside the VisionEnginePlugin.
...
private:
  EFFECTS_IMPEXP static DynArray_cl<ELEMTYPE> elementTable;           ///< static element table, used to store the elements
  EFFECTS_IMPEXP static unsigned int g_iElementCount;                 ///< current number of elements (returned by GetSize)
  EFFECTS_IMPEXP static unsigned int g_iFreeElementCache[];
  EFFECTS_IMPEXP static unsigned int g_iNumElementsInCache;
  EFFECTS_IMPEXP static unsigned int g_iCurrentElementCacheIndex;
  EFFECTS_IMPEXP static unsigned int g_iResizeGranularity;
...
#ifdef _VISION_WIIU
#define V_ENGINE_PLUGIN_ELEMENT_MANAGER_TEMPLATE_DECL(TYPE) \
  template<> static DynArray_cl<TYPE *> VEnginePluginElementManager<TYPE *>::elementTable; \
  template<> static unsigned int VEnginePluginElementManager<TYPE *>::g_iElementCount; \
  template<> static unsigned int VEnginePluginElementManager<TYPE *>::g_iFreeElementCache[V_ENGINE_PLUGIN_ELEMENT_MANAGER_FREE_INDEX_CACHE_SIZE]; \
  template<> static unsigned int VEnginePluginElementManager<TYPE *>::g_iNumElementsInCache; \
  template<> static unsigned int VEnginePluginElementManager<TYPE *>::g_iCurrentElementCacheIndex; \
  template<> static unsigned int VEnginePluginElementManager<TYPE *>::g_iResizeGranularity; \

#else
  #define V_ENGINE_PLUGIN_ELEMENT_MANAGER_TEMPLATE_DECL(TYPE) 
#endif

#define V_ENGINE_PLUGIN_ELEMENT_MANAGER_INIT_STATICS(TYPE) \
  template<> DynArray_cl<TYPE *> VEnginePluginElementManager<TYPE *>::elementTable(0, NULL); \
  template<> unsigned int VEnginePluginElementManager<TYPE *>::g_iElementCount = 0; \
  template<> unsigned int VEnginePluginElementManager<TYPE *>::g_iFreeElementCache[V_ENGINE_PLUGIN_ELEMENT_MANAGER_FREE_INDEX_CACHE_SIZE] = { 0 }; \
  template<> unsigned int VEnginePluginElementManager<TYPE *>::g_iNumElementsInCache = 0; \
  template<> unsigned int VEnginePluginElementManager<TYPE *>::g_iCurrentElementCacheIndex = 0; \
  template<> unsigned int VEnginePluginElementManager<TYPE *>::g_iResizeGranularity = 64; \

#include <Vision/Runtime/EnginePlugins/VisionEnginePlugin/Components/VEnginePluginElementManager.inl>
