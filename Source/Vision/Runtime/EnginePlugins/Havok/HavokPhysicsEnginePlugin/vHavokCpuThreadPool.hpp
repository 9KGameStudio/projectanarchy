...
/// The construction info for the vHavokCpuThreadPool
class VisCallback_cl;

/// \brief
///   Thread pool construction parameters.
struct vHavokCpuThreadPoolCinfo
{
  HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_UTILITIES, vHavokCpuThreadPoolCinfo );
...
class vHavokCpuThreadPool : public hkThreadPool
{
public:
  ...
  ///
  vHavokCpuThreadPool(const vHavokCpuThreadPoolCinfo& cinfo);
  ...
  /// \brief
  ///   Process tasks using all threads until all tasks on the queue have completed.
  ...
  /// \brief
  ///   Returns true if processing of jobs has started (using processJobQueue)
  ///   but waitForCompletion has not been called yet.
  ///
  /// \sa processJobQueue waitForCompletion
  virtual bool isProcessing() const HKV_OVERRIDE;

  /// \brief
  ///   Returns the timer data that has been collected during processJobQueue.
  ...
  // Enables triggering the WorkerThreadCreated callback the next time the job queue is processed.
  void setWorkerThreadCreatedFlag();
...
protected:
  ...
  static void HK_CALL jobQueueWorkerFunction(void* workLoad);
...
public:
  ...
  struct WorkerThreadData
  {
    ...
    /// brief
    ///   Triggers WorkerThreadCreated callback.
    bool m_triggerWorkerThreadCreated;
  };
  ...
  struct SharedThreadData
  {
    HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_UTILITIES, vHavokCpuThreadPool::SharedThreadData);
...
protected:
  ...
  ///   Thread stack size.
  ///
  /// \sa vHavokCpuThreadPoolCinfo::m_stackSize
  int m_stackSize;
};
