...
/// \brief
///   Base class for descriptors of Havok Physics constraint chains
///
/// This base class holds all the properties that are common to all 
/// constraint chain types in vHavokPhysics. A subclass exists for each concrete
/// Havok Physics constraint chain type, which may contain additional properties.
...
public:
  ...
  /// \brief
  ///   Returns the key of the path to fit the constraint chain to.
  /// \return
...
public:
  VSmartPtr<vHavokRigidBody> m_spAnchorBodies[2]; ///< The vHavokPhysics rigid bodies to anchor the ends of 
  ...
  float m_fLinkLinearDamping;                     ///< The linear damping for all links in this chain.
  float m_fLinkAngularDamping;                    ///< The angular damping for all links in this chain.
  ...
  bool m_bComputeCfm;                             ///< Determines whether to compute the CFM 
                                                  ///< (or to use a fixed CFM otherwise)
  float m_fCfmFactor;                             ///< Scale of computed CFM (constraint force mixing). 
                                                  ///< 0.0f: don't compute CFM and use Havok Physics' default.
...
public:
  ...
  /// \brief
  ///   Destructor.
  VHAVOK_IMPEXP virtual ~vHavokConstraintChain();
  ...
  ///
  /// \param desc
  ///   the vHavokPhysics constraint chain descriptor
...
public:
  ...
  VHAVOK_IMPEXP virtual void Step(float dt) HKV_OVERRIDE;
  ...
  /// \return
  ///   \c false if the creation of the Havok Physics constraint chain failed.
  inline bool IsValid() const 
  { 
    return (m_pConstraintChain != NULL);
  }
  ...
  /// \brief
  ///   Returns the object used internally by Havok Physics to manage the constraint chain.
  inline hkpConstraintChainInstance *GetHkConstraintChainInstance() const 
  { 
    return m_pConstraintChain; 
  }
  ...
  inline float GetLastLinkExtra() const 
  { 
  ...
  ///
  /// \param pRotations
  ///   Pointer to an array of rotation matrices. This array must be provided
  ///   by the caller and be at least of the size returned by GetNumLinks().
  /// \param pTranslations
  ///   Pointer to an array of translation vectors. This array must be provided
  ///   by the caller and be at least of the size returned by GetNumLinks().
  /// \param bRemoveRoll
  ///   Determines whether to remove the roll component in the output transforms.
  /// \param bReverseOrder
  ///   Determines whether to output the link transforms in reverse order, 
  ///   with a yaw offset of 180 degrees applied.
  ...
  ///   the path segments they approximate. Scaled so that 1.0 = link length.
  inline float GetApproximationError() const
  { 
    return m_fApproximationError; 
  }
  ...
  { 
    return m_vChainForce; 
  }

  /// \brief
  ///   Returns the length of the force currently in effect on this constraint chain.
  ///
  /// \return
  ///   The norm of the force on the constraint chain, or \c 0, if the constraint chain
  ///   is not initialized
  inline float GetChainForce() const 
  { 
    return m_vChainForce.getLength(); 
  }
...
protected:
  ...
  /// \brief
  ///   Returns the maximum number of links that this constraint chain is
  ///   allowed to have.
  ...
  ///
  VHAVOK_IMPEXP void RemoveLinkRoll(hkvMat3 const& mPrevRot, hkvMat3& mRot,
...
public:
  ...
  ///
  inline float GetChainLength() const 
  { 
    return m_fUnstretchedLength; 
  }
  ...
  VHAVOK_IMPEXP void constraintDeletedCallback(hkpConstraintInstance* constraint);
...
protected:
  ...
  ///
  VHAVOK_IMPEXP void CalcLinkError(hkvVec3 const& vEndAnchor,
  ...
  ///
  VHAVOK_IMPEXP unsigned int CalcPivotPoints(DynObjArray_cl<hkvVec3>& pivots);
  ...
  ///   the number of pivot points calculated
  ///
  VHAVOK_IMPEXP unsigned int CalcPivotPointsFromAnchors(DynObjArray_cl<hkvVec3>& pivots);
  ...
  ///   the number of pivot points calculated
  ///
  VHAVOK_IMPEXP unsigned int CalcPivotPointsFromPath(DynObjArray_cl<hkvVec3>& pivots, VisPath_cl* pPath);
  ...
  ///
  VHAVOK_IMPEXP void AdjustLastChainLinkPivot();
  ...
  ///   Must be overridden in derived classes!
  ///
  VHAVOK_IMPEXP virtual void AppendConstraintInfo( const hkvVec3& vPivotA, const hkvVec3& vPivotB, 
...
private:
  ...
  /// \brief
  ///   Removes the ragdoll constraint for the specified constraint.
  ///
  /// \param iConstraintIndex
...
protected:
  ...
  ///
  VHAVOK_IMPEXP hkpRigidBody *CreateChainLink(hkvVec3 vPivot1, hkvVec3 vPivot2);
  ...
  ///   its position and orientation.
  ///
  VHAVOK_IMPEXP hkpRigidBody *CreateChainLink(hkvVec3 const& vPosition, 
  ...
  ///
  VHAVOK_IMPEXP bool AddChainLink();
  ...
  ///
  VHAVOK_IMPEXP bool RemoveChainLink();
  ...
  ///
  VHAVOK_IMPEXP unsigned int CalcNumLinksRequired(float fLength) const;
  ...
  /// \brief
  ///   Creates the native Havok Physics constraint chain data instance for this type
  ...
  /// \brief
  ///   Creates a vHavok Physics constraint chain descriptor for this type of
  ///   constraint chains.
  ///
  /// \return  
  ...
  ///   \c true if successful, \c false if not
  ///
  /// This method is called form Init() and can be overridden in 
  ...
  /// \brief
  ///   Initializes a native Havok Physics constraint chain data object from a
  ///   vHavokPhysics constraint chain descriptor.
  ///
  /// \param data
  ///   the Havok Physics constraint chain data
  /// \param desc
  ///   the vHavokPhysics constraint chain descriptor
  ///
...
public:
  /// \brief
  ///   Saves the configuration of this constraint chain to a vHavokPhysics
...
protected:
  vHavokPhysicsModule *m_pModule;                   ///< The physics module instance that manages this 
                                                    ///< constraint chain.
  ...
  VSmartPtr<vHavokRigidBody> m_spAnchorBodies[2];   ///< The rigid bodies used as anchors for the constraint 
                                                    ///< chain ends,
  ...
  bool m_bCalculateLinkInertia;                     ///< Whether to let Havok calculate an inertia tensor for 
                                                    ///< the chain links.
  hkvVec3 m_vInertiaTensorAxisScaling;              ///< Scaling of the chain link inertia tensor axes.
  float m_fInertiaTensorUniformScaling;             ///< Uniform scaling of the chain link inertia tensor.
...
private:
  ...
  VString m_sChainPathKey;                          ///< The key of the path which the links of the constraint chain 
                                                    ///< must follow.
  DynArray_cl<hkpRigidBody *> m_LinkBodies;         ///< The list of Havok rigid bodies that are used for the chain 
                                                    ///< links. This does not include the anchor bodies at either end 
                                                    ///< of the chain.

  float m_fApproximationError;                      ///< The average error of the segment-wise approximation of the 
                                                    ///< path by the links of the chain.

  float m_fUnstretchedLength;                       ///< Length of the constraint chain when the chain links are 
                                                    ///< not pulled apart (in Vision units).
  float m_fLastLinkExtra;                           ///< Amount that needs to be compensated at the last link to 
                                                    ///< correct the chain length (in Vision units, may be positive 
                                                    ///< or negative).

  hkvVec3 m_vChainForce;                            ///< The force currently in effect on the chain.

  bool m_bDisposed;                                 ///< Determines whether this constraint chain has been disposed.
...
private:
  ...
  bool bWasInWorld;                   ///< Stores whether the constraint chain was removed 
                                      ///< from the world in the constructor.
};
