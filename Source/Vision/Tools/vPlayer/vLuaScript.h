...
public:
  ...
  unsigned int getNumberOfFunctionParameters() const;
...
inline unsigned int vLuaScript::getNumberOfFunctionParameters() const
{
...
inline bool vLuaScript::ExecuteFile(const char* szFile, VString& out_sError) const
{
  ...
  if (pFile == NULL)
  {
    out_sError.Format("File '%s' not found.", szFile);
    return false;
  }
  ...
  return ExecuteString(sBuffer, out_sError);
}  
