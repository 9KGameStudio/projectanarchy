...
#define CHECK_ARRAY_SIZE_AND_RETURN_IF_OUT_OF_MEM(_arrayName, _returnval) { if( (_arrayName).getSize() >= (_arrayName).getCapacity() )	return _returnval; }
#else
...
{	
	...
	/* Flood regions	*/ 
#ifdef HK_PLATFORM_SPU
	int numTriangles = m_mesh.m_triangles.getSize();
	
	// do better by sizing to min of free stack space and this value because we already have code to exit gracefully
	// when these stacks are exceeded.
	hkLocalArray<Edge>	stack1(3 * numTriangles);
	hkLocalArray<Edge>	stack2(3 * numTriangles);
	HK_SPU_STACK_POINTER_CHECK();
	hkLocalArray<Edge>*	current = &stack1;
	hkLocalArray<Edge>*	next    = &stack2;
#else
...
inline hkBool32												TRIANGULATOR_TYP::floodEdgeSets(const FLOODPOLICY& policy)
{
	...
	// do better by sizing to min of free stack space and this value because we already have code to exit gracefully
	// when these stacks are exceeded.
	hkLocalArray<EdgeSet> stack(3 * numTriangles);
#endif
	for(Triangle* t=m_mesh.m_triangles.getFirst();t;t=t->next())
	{
		for(int i=0;i<3;++i)
