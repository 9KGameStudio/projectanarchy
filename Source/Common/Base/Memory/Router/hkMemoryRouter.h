...
#ifdef HK_PLATFORM_SIM_SPU
#include <Common/Base/Memory/Util/Stack/hkSpuStack.h>
#endif
...
#if defined(HK_COMPILER_MWERKS) || defined(HK_COMPILER_ARMCC) 
...
// Deprecated functions
...
#ifdef HK_PLATFORM_SIM_SPU
	HK_FORCE_INLINE int hkStackQuerySize()
	{
		return hkMemoryRouter::getInstance().stack().getFreeStackSize();
	}
	...
	HK_FORCE_INLINE TYPE* HK_CALL hkAllocateStack(int n, const char* what=HK_NULL )
	{
		...
		return static_cast<TYPE*>(mem.stack().blockAlloc(HK_NEXT_MULTIPLE_OF(128,n*hkSizeOfTypeOrVoid<TYPE>::val)));
	}
	...
	{
		hkMemoryRouter::getInstance().stack().blockFree(static_cast<void*>(ptr), HK_NEXT_MULTIPLE_OF(128,n*hkSizeOfTypeOrVoid<TYPE>::val));
	}
	...
	HK_FORCE_INLINE void hkDeallocateStack(int numBytes)
	{
		hkMemoryRouter::getInstance().stack().deallocateStack(numBytes);
	}
#else
	template <typename TYPE>
	HK_FORCE_INLINE TYPE* HK_CALL hkAllocateStack(int n, const char* what=HK_NULL )
