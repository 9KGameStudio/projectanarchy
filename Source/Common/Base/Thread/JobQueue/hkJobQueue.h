...
class HK_EXPORT_COMMON hkJobQueue
{
	...
	#define MAX_NUM_THREAD_TYPES HK_JOB_TYPE_MAX + CUSTOM_JOB_TYPE_MAX + 2
#if defined HK_PLATFORM_SIM
	#define MAX_NUM_CPU_THREADS 4	// SPU SIM
#else
	#define MAX_NUM_CPU_THREADS 2	// SPU
#endif
#elif defined HK_PLATFORM_XBOX360
	#define MAX_NUM_THREAD_TYPES CUSTOM_JOB_TYPE_MAX + 1 + 1 // one for shared cache, one for master thread
	...
	#define MAX_NUM_CPU_THREADS HK_MAX_NUM_THREADS
		...
		struct DynamicData
		{
			...
			int m_numRunningSpus;

			JobTypeSpuStats m_spuStats[HK_JOB_TYPE_MAX];
	...
	public:
		...
		JobStatus processAllJobs( bool addTimers = true);
			...
			/// Register an elf for this job category
		void registerSpuElf( hkJobType jobType, void* spuElf );

			/// Registers an SPU overlay table
		void registerSpuOverlayTable( hkJobType jobType, void* overlayTable );

			/// Used to determine the correct elf to initially load
		void* getInitialElf();
		...
		// No implementation, just to silence compiler warnings about missing constructors.
		hkJobQueue( const hkJobQueueCinfo& cinfo );
...
#if defined(HK_PLATFORM_HAS_SPU)
		// PPU only
		void* m_spuElfs[HK_JOB_TYPE_MAX];
		void* m_spuOverlays[HK_JOB_TYPE_MAX];
		static hkJobQueue* s_instance;	///< instance of the jobqueue, needed to flag the SPUs to get out of memory state
#	if defined(HK_PLATFORM_SPU)
		static bool m_outOfMemory;
#	endif
#endif
