...
: hkJob( HK_JOB_TYPE_RAYCAST_QUERY, subType, size, HK_JOB_SPU_TYPE_INVALID ),
#else
...
:	hkpRayCastQueryJob(RAYCAST_QUERY_SHAPE_RAYCAST, sizeof(hkpShapeRayCastJob))
{
	...
	hkSpuDmaUtils::setInt32InMainMemory( &m_sharedJobHeaderOnPpu->m_openJobs, ((numCommands-1)/numCommandsPerTask) + 1 );
#else
...
//	HK_ASSERT2( 0xaf3647f6, numCommands > 0,																		"numCommands has to be > 0." );
	...
	// precalculate the total number of jobs that will be spawned from the original job (incl. the original)
#ifdef HK_PLATFORM_SPU
	hkSpuDmaUtils::setInt32InMainMemory( &m_sharedJobHeaderOnPpu->m_openJobs, (numCommands==0)? 1 : ((numCommands-1)/numCommandsPerTask) + 1);
#else
