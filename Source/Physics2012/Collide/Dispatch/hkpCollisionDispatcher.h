...
#else
class hkCollisionDispatcherPpu : public hkReferencedObject
#endif
{
	public:
...
#else
			HK_MAX_AGENT3_TYPES = 16,	// this must be a power of 2
#endif
	...
	/// SPU version of collision dispatcher.
	/// Uses a smaller dispatch table than the PPU version, handling only the shapes that are supported on SPU.
	/// Dispatches only to "agent3" types.
class hkpCollisionDispatcher
{
	public:

		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_COLLIDE, hkpCollisionDispatcher );

		enum {
			HK_MAX_RESPONSE_TYPE = hkCollisionDispatcherPpu::HK_MAX_RESPONSE_TYPE,
			HK_MAX_SHAPE_TYPE    = hkcdShapeType::MAX_SPU_SHAPE_TYPE,
			HK_MAX_AGENT3_TYPES  = hkCollisionDispatcherPpu::HK_MAX_AGENT3_TYPES,
			HK_MAX_COLLISION_QUALITIES = hkCollisionDispatcherPpu::HK_MAX_COLLISION_QUALITIES
		};
	
		struct Agent3Funcs
		{
			HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_COLLIDE, hkpCollisionDispatcher::Agent3Funcs );

			Agent3Funcs() : m_isPredictive(false) {}
			...
			hkAgent3::ProcessFunc	m_processFunc;
			hkBool					m_isPredictive;
			// on PPU this is in hkAgent3Internal
			hkAgent3::Symmetric		m_symmetric;
		};

	public:

		typedef hkCollisionDispatcherPpu::CollisionQualityIndex CollisionQualityIndex;
		typedef Agent3Funcs Agent3FuncsIntern;
	...
	public:
		...
		HK_ALIGN16( hkUchar		m_agent3Types		[HK_MAX_SHAPE_TYPE][HK_MAX_SHAPE_TYPE] );
		HK_ALIGN16( hkUchar		m_agent3TypesPred	[HK_MAX_SHAPE_TYPE][HK_MAX_SHAPE_TYPE] );
		HK_ALIGN16( hkUint32	m_hasAlternateType	[HK_MAX_SHAPE_TYPE*2] );
		Agent3FuncsIntern       m_agent3Func		[HK_MAX_AGENT3_TYPES];
	...
	/// SPU version of collision dispatcher, used only for linear cast and get closest point queries.
	/// Dispatches only to "agent2" types.
class hkpSpuCollisionQueryDispatcher
{
	public:

		HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_COLLIDE, hkpSpuCollisionQueryDispatcher );

		enum
		{
			HK_MAX_SHAPE_TYPE    = hkcdShapeType::MAX_SPU_SHAPE_TYPE,
			HK_MAX_AGENT2_TYPES  = hkCollisionDispatcherPpu::HK_MAX_AGENT2_TYPES,
		};
		...
		struct AgentFuncs
		{
			HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR( HK_MEMORY_CLASS_COLLIDE, hkpSpuCollisionQueryDispatcher::AgentFuncs );
