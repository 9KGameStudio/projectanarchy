	...
	public:
		...
		struct OrientationCacheBase
		{
			public:
				...
				HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_DESTRUCTION, OrientationCacheBase);

			public:

				/// Constructor. Initializes the multiplier with 4 large prime numbers
				HK_FORCE_INLINE OrientationCacheBase();

#ifdef HK_DEBUG

				/// Gets the ratio of cache hit
				HK_FORCE_INLINE hkReal getCacheHitRatio() const;

				/// Resets cache hit counters
				HK_FORCE_INLINE void resetCacheHitCounters();
#endif
